...
today_str = int(today.strftime("%d")) # 10
today_name = today.strftime("%A") # Saturday
today_year = int(today.strftime("%Y")) # 2018
	...
	for m in range(1,13):
		...
		for d in range(1,int(days_of_month) + 1):
			...
			if d == today_str: 
				if today_name == day:
					sum_day += 1
print "There will be ", sum_day , today_name + "s" , "which will be the ", str(today_str) + "th", "of the month , in the next 10 years."